      ...
      <section className="panel table-panel">
        <div className="table-scroll"><table><thead><tr><th>Nhân sự</th><th>Vai trò</th><th>Phạm vi</th><th>Quyền</th><th>Hoạt động</th><th /></tr></thead><tbody>
          {users.map((person) => <tr key={person.id}>
            ...
            <td><div className="permission-list"><span className={person.can_add_accounts || person.role === "BOSS" ? "on" : ""}>Thêm</span><span className={person.can_delete_accounts || person.role === "BOSS" ? "on" : ""}>Xóa</span><span className={person.can_run_checks || person.role === "BOSS" ? "on" : ""}>Check</span></div></td>
            ...
            <td>{(person.role !== "BOSS" || current?.is_system_owner) && <button className="icon-button" onClick={() => setEditing(person)}><Edit3 size={17} /></button>}</td>
          </tr>)}
        </tbody></table></div>
      </section>
...
function UserDialog({ user, users, current, onClose, onSaved }: { user: User | null; users: User[]; current: User; onClose: () => void; onSaved: () => void }) {
  ...
  return <Modal title={user ? `Sửa ${user.full_name}` : "Thêm tài khoản"} onClose={onClose}><form className="form-stack" onSubmit={(e) => { e.preventDefault(); save.mutate(); }}><div className="form-grid">
    ...
    {!user && <label>Vai trò<select value={role} onChange={(e) => setRole(e.target.value as Role)}><option value="LEADER">LEADER</option><option value="MEMBER">MEMBER</option>{current.is_system_owner && <option value="BOSS">BOSS</option>}</select></label>}
