...
export function OrganizationPage() {
  ...
  return (
    <div className="page-stack">
      <div className="page-heading"><div><p className="eyebrow">TỔ CHỨC NHÂN SỰ</p><h1>Sơ đồ công ty</h1><p>Quan hệ BOSS → Quản lý → Leader → Member theo đúng phạm vi được phép xem.</p></div><span className="heading-symbol"><Network size={24} /></span></div>
            ...
            <div className="org-level org-boss-level">
              {bosses.map((person) => (
                <PersonCard
                  key={person.id}
                  person={person}
                  onOpen={() => navigate(`/accounts?owner=${person.id}`)}
                />
              ))}
            </div>
          )}
          ...
          <div className="org-branches">
            {leaders.map((leader) => (
              <div className="org-branch" key={leader.id}>
                ...
                <PersonCard
                  person={leader}
                  onOpen={() => navigate(`/accounts?owner=${leader.id}`)}
                />

                <div className="member-grid">
                  {members
                    .filter((member) => member.leader_id === leader.id)
                    .map((member) => (
                      <PersonCard
                        key={member.id}
                        person={member}
                        onOpen={() =>
                          navigate(`/accounts?owner=${member.id}`)
                        }
                      />
                    ))}
