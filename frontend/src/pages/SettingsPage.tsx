...
export function SettingsPage() {
  ...
  const canManageTotpRestriction = user?.role === "BOSS" || user?.role === "MANAGER";
  ...
  return <div className="page-stack"><div className="page-heading"><div><p className="eyebrow">HỆ THỐNG</p><h1>Cấu hình checker</h1><p>Giới hạn tài nguyên chung cho tất cả job đang chạy.</p></div><span className="heading-symbol"><Settings2 size={24} /></span></div><form className="panel settings-form" onSubmit={(e) => { e.preventDefault(); save.mutate(); }}><div className="settings-grid">{numberField("check_interval_minutes", "Chu kỳ tự động (phút)", 15, 1440)}{numberField("follower_change_threshold", "Ngưỡng thay đổi follow", 1)}{numberField("monetization_follower_threshold", "Ngưỡng hiện nút BKT", 1, 1000000000)}{numberField("max_total_workers", "Tổng worker", 1, 50)}{numberField("max_workers_per_job", "Worker mỗi job", 1, 20)}{numberField("request_timeout_seconds", "Timeout (giây)", 3, 120)}{numberField("retry_count", "Retry lỗi mạng", 0, 5)}{numberField("dead_confirmation_attempts", "Số lần xác nhận DIE", 2, 5)}{numberField("request_delay_seconds", "Delay request (giây)", 0, 60, .1)}</div><div className="checkbox-grid settings-checks"><span>Check toàn công ty tự động đang tạm tắt; hãy check theo từng người.</span><label><input type="checkbox" checked={Boolean(form.in_app_notifications_enabled)} onChange={(e) => update("in_app_notifications_enabled", e.target.checked)} /> Thông báo trong web</label><label><input type="checkbox" checked={Boolean(form.voice_notifications_enabled)} onChange={(e) => update("voice_notifications_enabled", e.target.checked)} /> Cho phép đọc loa</label></div><div className="settings-footer"><span>Múi giờ: {form.timezone} · Lần chạy kế: {formatTime(form.next_auto_check_at)}</span><button className="button primary" disabled={save.isPending}>Lưu cấu hình checker</button></div></form>{canManageTotpRestriction && <form className="panel totp-restriction-settings" onSubmit={(e) => { e.preventDefault(); save.mutate(); }}><div className="totp-restriction-heading"><div><p className="eyebrow">BẢO MẬT TÀI KHOẢN</p><h2>Giới hạn quyền dùng 2FA</h2><small>Server dùng cố định múi giờ Việt Nam, không dựa vào giờ của máy nhân viên.</small></div><label className="toggle-switch"><input type="checkbox" checked={Boolean(form.totp_time_restriction_enabled)} onChange={(e) => update("totp_time_restriction_enabled", e.target.checked)} /><span className="toggle-track" /><span className="toggle-label">{form.totp_time_restriction_enabled ? "Đang bật" : "Đang tắt"}</span></label></div><div className="settings-grid totp-restriction-grid"><label>Áp dụng cho<select value={form.totp_restricted_roles || "MEMBER"} onChange={(e) => update("totp_restricted_roles", e.target.value as "MEMBER" | "MEMBER_AND_LEADER")}><option value="MEMBER">Chỉ Member</option><option value="MEMBER_AND_LEADER">Member và Leader</option></select></label><label>Bắt đầu (giờ Việt Nam)<input type="time" value={toTimeValue(form.totp_access_start_minutes)} onChange={(e) => updateTime("totp_access_start_minutes", e.target.value)} /></label><label>Kết thúc (giờ Việt Nam)<input type="time" value={toTimeValue(form.totp_access_end_minutes)} onChange={(e) => updateTime("totp_access_end_minutes", e.target.value)} /></label></div><div className="totp-restriction-footer"><small>BOSS và MANAGER luôn có quyền sử dụng 2FA.</small><button className="button primary" disabled={save.isPending}>Lưu giới hạn 2FA</button></div></form>}</div>;
}
